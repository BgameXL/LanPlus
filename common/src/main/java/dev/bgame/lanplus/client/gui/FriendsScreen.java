...
public final class FriendsScreen extends Screen {
    ...
    public void removed() {
        FriendsService fs = LanPlusClient.friends();
        if (fs != null) {
            fs.removeListener(changeListener);
        }
    }
    ...
    public void render(GuiGraphics g, int mouseX, int mouseY, float partialTick) {
        ...
        if (status != null) {
            if (System.currentTimeMillis() < statusUntil) {
                g.drawString(this.font, status, contentRight - 8 - this.font.width(status), paneBottom + 10, LanPlusUI.MUTED);
            } else {
                status = null;
            }
        }
    ...
    private void renderTabs(GuiGraphics g, int mouseX, int mouseY) {
        for (int i = 0; i < TABS.length; i++) {
            ...
            if (active) {
                g.fill(x, tabsTop + 11, x + w, tabsTop + 12, LanPlusUI.LIME);
            }
    ...
    private void drawTabBadge(GuiGraphics g, int x, int y, int count) {
        ...
        int w = tw + 4;
        g.fill(x, y - 1, x + w, y + 8, LanPlusUI.LIME);
        g.pose().pushPose();
        g.pose().translate(x + (w - tw) / 2.0f, y + 0.5f, 0);
        g.pose().scale(scale, scale, 1f);
        g.drawString(this.font, s, 0, 0, LanPlusUI.EDGE_DARK, false);
        g.pose().popPose();
    }
    ...
    private void renderRequests(GuiGraphics g, int paneBottom) {
        ...
        for (ResolvedUser r : reqs) {
            ...
            g.fill(ax, y + 2, ax + 18, y + 16, LanPlusUI.ONLINE);
    ...
    private void renderFeed(GuiGraphics g, int x, int w, int y) {
        ...
        if (feed.isEmpty()) {
            g.drawString(this.font, Component.translatable("gui.lanplus.hub.recent.empty"),
                    x + 8, y + 4, LanPlusUI.FAINT, false);
            return;
        }
    ...
    private Component feedLine(ActivityEntry e) {
        String actor = e.actorName() == null ? "?" : e.actorName();
        String subject = e.subject() == null ? "?" : e.subject();
        return switch (e.type()) {
            case HOSTING_STARTED -> Component.translatable("gui.lanplus.feed.hosting", actor, subject);
            case FRIEND_ADDED -> Component.translatable("gui.lanplus.feed.friend", actor, subject);
            case UNKNOWN -> Component.literal(actor);
        };
    }
    ...
    private void renderContextMenu(GuiGraphics g, int mouseX, int mouseY) {
        ...
        for (ContextEntry e : contextEntries) {
            ...
            if (hover && e.enabled()) {
                g.fill(x + 1, ey, x + w - 1, ey + MENU_ROW_H, LanPlusUI.ACCENT_TINT);
            }
    ...
    private void doJoin() {
        joinFriend(selectedFriend());
    }

    private void joinFriend(Friend friend) {
        InviteService invites = LanPlusClient.invites();
